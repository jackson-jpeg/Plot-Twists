              ...
              <AnimatePresence>
                ...
                {roomPreview && !isLoadingPreview && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                    ...
                    {roomPreview.playerCount >= roomPreview.maxPlayers && (
                      <motion.p className="text-xs text-center p-2 rounded" style={{ background: 'rgba(255,255,255,0.5)', color: 'var(--color-warning)' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>👁️ Room is full — you'll join as a spectator</motion.p>
                    )}
                    {roomPreview.gameState !== 'LOBBY' && (
                      <motion.p className="text-xs text-center p-2 rounded mt-2" style={{ background: 'rgba(255,255,255,0.5)', color: 'var(--color-danger)' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>⚠️ Game already in progress — wait for next round</motion.p>
                    )}
                  </motion.div>
                )}
              </AnimatePresence>
