...
async function main() {
  ...
  if (real) {
    ...
    p()
    p('**One thing worth flagging that is not about money.** The prompt asks for 30–40 lines and')
    p('these came back at 45–64. Nothing enforces the range, so scripts run long. At a party that')
    p('is a real duration question rather than a cost one — 64 lines is a lot of performing.')
    p()
  }
